/**
 * src/protocol/decode.js — MessagePack DECODER for Colonist.io's wire protocol.
 *
 * VERIFIED encoding (see the repo's NOTES.md §2): the socket carries MessagePack, not JSON.
 * This is a compact, dependency-free MessagePack decoder covering exactly the types Colonist
 * uses (maps, arrays, strings, ints, floats, bool, null, bin, and the timestamp ext). It was
 * validated byte-for-byte against @msgpack/msgpack over every captured frame.
 *
 * No DOM, no Node APIs, no external imports. Runs identically in a browser and in Node.
 * TextDecoder is feature-detected (present in both modern environments).
 */
