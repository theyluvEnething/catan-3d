/**
 * state/watchdog.js — desync watchdog.
 *
  ...
  state._applySnapshot = function (payload) {
    ...
    if (state.ready && priorBoard && incoming) {
      ...
      if (drifts.length) {
        ...
        try { console.warn("[catan-interface/watchdog] DESYNC detected —", drifts.length, "field(s):", JSON.stringify(drifts.slice(0, 6))); } catch {}
      }
    }
