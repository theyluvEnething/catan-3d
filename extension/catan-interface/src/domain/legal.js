/**
 * src/domain/legal.js — legal-move computation from the reconstructed GameState.
 *
 * Pure functions (no DOM). Given the mapState, compute which corners/edges/hexes are legal
 * for the current player+context, so a UI can highlight only valid targets and the direct-send
 * layer never emits an illegal action. Uses boardGeometry adjacency.
 *
 * Rules (base Catan):
 ...
 */
import { cornerEdges, edgeCorners, hexCorners } from "./boardGeometry.js";
